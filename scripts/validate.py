...
def validate_schemes(path=None, today=None):
    ...
    # The MSME playbook quotes fees and legal limits, so it needs the same
    # forced review — an out-of-date fee sends a small business to the wrong
    # place with the wrong money.
    mpath = os.path.join(OUT, "vc_msme.js")
    if os.path.exists(mpath):
        with open(mpath, encoding="utf-8") as f:
            mtext = f.read()
        manchor = "window.VC_DATA.msme ="
        mbody = mtext[mtext.index(manchor) + len(manchor):mtext.rindex(";")]
        mbody = _re.sub(r"/\*.*?\*/", "", mbody, flags=_re.S)
        mobj = json.loads(mbody.strip())
        rb = mobj.get("review_by")
        if not rb:
            problems.append("msme: no review_by date set")
        else:
            try:
                d = datetime.date(*[int(x) for x in rb.split("-")])
                if d < today:
                    problems.append(
                        f"msme playbook: due for review since {rb}. Re-check fees, "
                        f"limits and portal links, then set a new review_by.")
            except ValueError:
                problems.append(f"msme: unreadable review_by {rb!r}")
