...
msme.html                MSME playbook: how to actually start exporting, step by step
...
js/msme.js               Charts and steps for the MSME playbook
...
Three robots do the remembering for you. All live in `.github/workflows/`
and run on GitHub's servers — nothing runs on your computer.

**1. `refresh-data.yml` — the weekly data refresh.**
Every Monday it re-fetches everything from UN Comtrade, rebuilds the pages,
and pushes if anything changed; Vercel then redeploys. Comtrade
...
* every day, this workflow re-checks those dates *and* fingerprints the
  official DGFT, Commerce and MSME scheme pages. If a date has lapsed or a
  page has changed, it opens an issue listing exactly what to look at. The
  last-seen fingerprint is committed, so you get one issue per real change,
  not one per run.
...
DGCIS publishes PDFs, not an API, so the States page can't be automated.
Every day this checks the DGCIS publications page and, when a new
...
responses in `data/raw/`, and rewrites `data/vc_core.js`, `data/vc_h2h.js`
and `data/vc_products.js` (plus the JSON files).
...
Monday and pushes the fresh numbers itself — Vercel then redeploys. You
can also trigger it any time from GitHub → Actions →
"Refresh trade data" → Run workflow. If Comtrade is down it fails safely,
GitHub emails you, and the site keeps serving the old data. The hand-curated
files (data/vc_states.js, data/vc_policies.js and the agreement cards in
